 ...
 * File Created: Friday, 10th August 2018 10:26:23 pm
 * Author: Navi Ocean (navi.ocean@outlook.com)
 * -----
 * Last Modified: Friday, 10th August 2018 10:27:05 pm
...
function cloneIfNecessary(value, optionsArgument) {
  ...
  return clone && isMergeableObject(value)
    ? deepmerge(
      emptyTarget(value), value, optionsArgument,
    )
    : value;
}
...
function defaultArrayMerge(
  ...
  source.forEach((e, i) => {
    if (typeof destination[i] === 'undefined') {
      ...
      destination[i] = deepmerge(
        target[i], e, optionsArgument,
      );
  ...
  Object.keys(source).forEach((key) => {
    if (!isMergeableObject(source[key]) || !target[key]) {
      ...
      destination[key] = deepmerge(
        target[key], source[key], optionsArgument,
      );
    }
  });
